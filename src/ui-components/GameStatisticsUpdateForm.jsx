...
export default function GameStatisticsUpdateForm(props) {
  const {
    id: idProp,
    gameStatistics,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = gameStatisticsRecord
      ? { ...initialValues, ...gameStatisticsRecord }
      : initialValues;
  ...
  const [gameStatisticsRecord, setGameStatisticsRecord] =
    React.useState(gameStatistics);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(GameStatistics, idProp)
        : gameStatistics;
      setGameStatisticsRecord(record);
    };
    queryData();
  }, [idProp, gameStatistics]);
  React.useEffect(resetStateValues, [gameStatisticsRecord]);
  const validations = {
    winDistribution: [],
    gamesFailed: [],
    currentStreak: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            GameStatistics.copyOf(gameStatisticsRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "GameStatisticsUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || gameStatistics)}
